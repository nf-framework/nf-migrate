...
class NFMigDb {
    ...
    async grantAll() {
        let grantAllFunction = config?.['@nfjs/migrate']?.grantAllFunction ?? 'nfc.f_db8grant_all';
        const argv = common.getPath(api, 'argv');
        if ('migrate-grant-all-function' in argv) grantAllFunction = argv['migrate-grant-all-function'];
        if (grantAllFunction === false) return Promise.resolve();
    ...
    async dbfwInit(appAdminName, appAdminPass, appAdminRole) {
        ...
        if (exists) {
            const options = {
                logonKind: this.provider.config?.connectType ?? 'user'
            }
            return this.query('select nfc.f_init($1,$2,$3,$4)', [appAdminName, appAdminPass, appAdminRole, options], 'adminConnect');
        }
        return Promise.resolve();
    }
