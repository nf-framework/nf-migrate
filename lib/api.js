...
async function checkAndRun(isConsoleRun) {
    ...
    if (needRunMig || needRunObj || needRunDat) {
        ...
        if (runChoice && runChoice !== 'm') {
            ...
            // выдача прав для adminConnect для создания объектов в схеме public в postgres версии 15+
            // проверка версии
            const versionQuery = await dbMig.query(
                `select split_part(setting,'.',1) major from pg_catalog.pg_settings where name = 'server_version';`,
                [],
                'checkConnect'
            );
            const version = versionQuery.rows?.[0]?.major;
            if (Number(version) >= 15) {
                sysScripts.push(`grant create on schema public TO ${dbMig.adminCredentials.user};`);
            }
            if (sysScripts.length > 0) {
                ...
                // ВАЖНО изменения системных объектов применяться даже если режим тестирования или в обычном режиме возникнет ошибка
